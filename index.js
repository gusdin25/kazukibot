/*
]=====> GUSDIN <=====[ ]=====> IG gusdin_ <=====[ ]=====> 081997826183 <=====[
*/
const {
   WAConnection,
   MessageType,
...
console.log(banner.string)
   ...
   qrcode.generate(qr, { small: true })
	console.log(color('[','white'), color('!','red'), color(']','white'), color(' FOLLOW IG gusdin_'))
})
...
kev.on('group-participants-update', async (anu) => {
	...
	kev.on('message-new', async (mek) => {
		try {
			...
			const tijep = moment.tz('Asia/Jakarta').format('DD/MM HH:mm:ss')
...
//AFK NGULI
	if (isGroup) {
                 if (checkAfkUser(mek.mentionedJid, _afk)) {
                    const getId = getAfkId(mek.mentionedJid, _afk)
...
switch(command) {
				...
				if (!isRegistered) return reply(ind.noregis())
					...
					await costum(help(pushname, prefix, botName, ownerName, time, ucapanSalam, kyun, uptime, reqXp, totalchat, getLevelingLevel, sender, _registered, uangku), text, tescuk, cr)
					break
				case 'dudvdb':
				reply(ucapanSalam) 
				break
				...
				case 'kevgrup':
				case 'kevgroup':
					kev.sendMessage(from, kevgrup(pushname, prefix, botName, ownerName), text)
					break
				case 'bingung':
				case 'bingungcok':
				if (isBanned) return reply(ind.baned())
    ...
    case 'slowvid':
			...
			break 
                ...
                if (!isRegistered) return reply(ind.noregis())
                    ...
                    kev.sendMessage('6281997826183@s.whatsapp.net', options, text, {quoted: mek})
				...
				case 'brainly':
					if (args.length < 1) return reply('Yang mau di cari apaan? titit?')
					anu = await fetchJson(`https://api.zeks.xyz/api/brainly?apikey=apivinz&q=${body.slice(8)}&count=8`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					srch = '=================\n'
...
*/
				...
				if (!isNsfw) return reply(ind.nsfwoff())
					kev.updatePresence(from, Presence.composing)
					if (args.length < 1) return reply('kode nya mana kak')
					anu = await fetchJson(`http://api.lolhuman.xyz/api/nhentai/${body.slice(12)}?apikey=e82fc9ea9c13cbb2b9c1be20`)
			if (anu.error) return reply(anu.error)
			...
			kev.sendMessage(from, thumb, image, {quoted: mek, caption: sket})
				case 'nhentaisearch':
					if (args.length < 1) return reply('Yang mau di cari apaan? titit?')
					if (!isNsfw) return reply(ind.nsfwoff())
					anu = await fetchJson(`http://api.lolhuman.xyz/api/nhentaisearch?apikey=e82fc9ea9c13cbb2b9c1be20&query=${body.slice(15)}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					hsrch = '=================\n'
					for (let i of anu.result) {
				...
				kev.sendMessage(from, ''+tes+'\n\n_By : ⸸Gusdin⸸Panutanque._', text, { quoted: mek })
				await limitAdd(sender)
				break				
				case 'ninjalogo':
                  if (isBanned) return reply(ind.baned())
				...
				reply(ind.wait())
				buffer = await getBuffer(`http://api.lolhuman.xyz/api/textprome2/ninjalogo?apikey=e82fc9ea9c13cbb2b9c1be20&text1=${nin}&text2=${ja}`)
		...
		case 'halloweentext':
		    ...
		    break
				...
				kh = body.slice(11)
				reply(ind.wait())
				anu = await getBuffer(`https://api.zeks.xyz/api/tpantai?text=${kh}&apikey=apivinz`)
				buffer = await getBuffer(anu.result.picture)
				...
				if (!isRegistered) return reply(ind.noregis())
					...
					reply(ind.wait())
					buffer = await getBuffer(`http://api.lolhuman.xyz/api/textprome2/glitch?apikey=847de7716f17a51eeba4235c&text1=${gli}&text2=${tch}`)
					kev.sendMessage(from, buffer, image, {quoted: mek})
					await limitAdd(sender)
					break
				case '8bit':
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return kev.sendMessage(from, 'Kode bahasanya mana kak? contoh : ${prefix}tts id Halo gusdin', text, {quoted: mek})
            ...
            break
               ...
               case 'botstat':
                ...
                var gh = body.slice(12)
					...
					var wm = gh.split("&")[1];
                    var tm = gh.split("&")[2];
					const pref = `yang mau dijadiin quote apaan, titit?\n\ncontoh : ${prefix}bikinquote aku bukan boneka & owner & backgroundnya`
					if (args.length < 1) return reply(pref)
					reply(ind.wait())
					anu = await fetchJson(`https://terhambar.com/aw/qts/?kata=${quote}&author=${wm}&tipe=${tm}`, {method: 'get'})
					buffer = await getBuffer(anu.result)
...
*/
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					var iniresultjawaban = `${anu.result.jawaban}`
					jawab1 = iniresultjawaban
					...
					setTimeout( () => {
					kev.sendMessage(from, ngebuff, image, { caption: '_Tebak bro!!! gak bisa jawab donasi ya:v_', quoted: mek }) // ur cods
					kev.sendMessage(from, iniresultjawaban.replace(/a|u|e|o/g, "-"), text, {quoted: mek})
					}, 0) // 1000 = 1s,
					await limitAdd(sender) 
					break
					case 'jawab':
...
var secondss = Math.floor((distance % (1000 * 60)) / 1000);
				...
				case 'gplaybutton':
				      if (isBanned) return reply(ind.baned())
				if (!isRegistered) return reply(ind.noregis())
if (args.length < 1) return reply(`「❗」Contoh : ${prefix}blackpink kazuki`)
gld = body.slice(16)
reply('「❗」Hmmmm -_-')
pbt = await getBuffer(`https://api.zeks.xyz/api/gplaybutton?text=${gld}&apikey=apivinz`)
...
slv = body.slice(18)
reply('「❗」Hah halu lu')
pbn = await getBuffer(`https://api.zeks.xyz/api/splaybutton?text=${slv}&apikey=apivinz`)
...
if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
  ...
  tels = body.slice(7)
  anu = await imgbb("cedeb44b8d204947a6833ca1412ca77d", owgi)
  hehe = await getBuffer(`https://api.zeks.xyz/api/slap?apikey=apivinz&img1=${anu.display_url}&img2=${anu.display_url}`)
 kev.sendMessage(from, hehe, image, {quoted:mek})
} else {
...
*/
		...
		case 'igvideo':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					if (args.length < 1) return reply('Urlnya mana um?')
					anu = await fetchJson(`https://onlydevcity.herokuapp.com/api/ig?url=https://www.instagram.com/p/CL4-cdRhbwh/?igshid=c66gzjn2o825&apikey=kevzka`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					teks = `*Title* : ${anu.hasil.result.name}\n*Size* : ${anu.hasil.result.likes}`
					buffer = await getBuffer(anu.hasil.result.data)
					kev.sendMessage(from, buffer, video, {mimetype: 'video/mp4', filename: `${anu.hasil.result.title}.mp4`, quoted: mek, caption: teks})
					await limitAdd(sender)
					break
		case 'tiktod':
		case 'tiktok':
		...
		hem = await fetchJson(`https://api.zeks.xyz/api/tiktok?url=${linktod}&apikey=apivinz`)
		reply(ind.download)
		rmln = await getBuffer(hem.watermark) // custom (hem.no_watermark)
	    kev.sendMessage(from, rmln, video, {mimetype: 'video/mp4', quoted: mek})
		await limitAdd(sender)
		break
		case 'joox':
			    if (isBanned) return reply(ind.baned())    
...
*/
                ...
                addKoinUser('6281997826183@s.whatsapp.net', fee)
				...
				break
				case 'iniapa':
				kev.sendMessage(from, `hai`, text, { quoted: cr})
				break
				case 'addlimit':
                  if (isBanned) return reply(ind.baned())				
				if (!isRegistered) return reply(ind.noregis())
...
*/				
            	case 'mining':
                      ...
                      if (!isEventon) return reply(`maaf ${pushname} event mining tidak di aktifkan sama owner GusDin`)
                      if (isOwner) {
                      const one = 999999999
                      addLevelingXp(sender, one)
                      addLevelingLevel(sender, 99)
                      reply(`karena GusDin baik Bot memberikan ${one}Xp >_<`)
                      }else{
...
case 'sandtext':
					...
					vh = body.slice(10)
					xh = await getBuffer(`https://api.zeks.xyz/api/sandw?apikey=apivinz&text=${vh}`) 
...
*/
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							kev.sendMessage(_.jid, buff, image, {caption: `*「 KAZUKI BROADCAST 」*\n\n${body.slice(4)}`})
						}
						reply('')
					} else {
						for (let _ of anu) {
